...
wait_for_request() {
  ...
  for ((i = 1; i <= attempts; i++)); do
    ...
    if python3 - "${requests_file}" "${expected_path}" "${expected_severity}" "${expected_kind}" "${expected_title}" <<'PY'
import json
import sys

requests_file, expected_path, expected_severity, expected_kind, expected_title = sys.argv[1:]
...
create_approval() {
  ...
  local incident_severity="$4"
  local workflow_name="$5"
  local escalation_kind="$6"
  local postmortem_required="$7"
...
approve_approval() {
  ...
  response="$(
    ...
    -d '{"status":"approved","comment":"Heimdall escalation smoke approval"}' \
    -w '\n%{http_code}'
  )"
  body="${response%$'\n'*}"
  status="${response##*$'\n'}"
...
echo "Dispatching Heimdall issue escalation..."
create_approval "${issue_approval}" "${issue_title}" "oom_killed" "high" "incident-escalation.yml" "issue" "false"
approve_approval "${issue_approval}"
wait_for_request \
...
echo "Dispatching Heimdall postmortem escalation..."
create_approval "${postmortem_approval}" "${postmortem_title}" "oom_killed" "critical" "postmortem.yml" "postmortem" "true"
approve_approval "${postmortem_approval}"
wait_for_request \
...
echo "Heimdall escalation smoke passed."
