...
- use `task install:smoke` para validar, em um workspace isolado, que as surfaces de referência e todo o catálogo remoto de integrations instalam corretamente; o smoke também valida o compose agregado, sobe um subconjunto representativo de runtime dentro do monorepo e executa o fluxo end-to-end de `core + auth + console`, incluindo leituras e escritas mínimas pela entrada `/api` do console
